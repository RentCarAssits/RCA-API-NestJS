...
export class PlanApplicationService{
    ...
    async register(registerPlanRequest: RegisterPlanRequest): Promise<Result<AppNotification,RegisterPlanRequest>>{
        const notification: AppNotification = await this.RegisterPlanValidator.validate(registerPlanRequest);
        if(notification.hasErrors()){
            return Result.error(notification);
        }
        const registerPlan: RegisterPlanRequest = new RegisterPlanRequest(
            registerPlanRequest.PlanId,
            registerPlanRequest.PlanName,
            registerPlanRequest.Benefits
        );

        const PlanId: number = await this.commandBus.execute(registerPlan);
        const registerPlanResponse: RegisterPlanResponse 
        = new  RegisterPlanResponse(
            PlanId,
